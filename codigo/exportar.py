...
from conversion import dias_en_semana_de_cinco
...
CONVERSION = """
    SELECT j.iso3                                            AS iso3,
           ...
           COALESCE(v.base_semanal_origen, 'no declarada por la norma')
                                                             AS base_segun,
           -- LA REGLA NO SE REESCRIBE AQUI. Estuvo escrita en SQL, cableando
           -- una base de cinco para `habil` y de seis para `werktage`, y
           -- divergio de la version en Python: Colombia salia 15,0 en este
           -- archivo y 12,5 en el informe. Ahora el SQL LLAMA a la unica
           -- implementacion, registrada como funcion de SQLite.
           dias_en_semana_de_cinco(v.texto_legal_dias, v.tipo_de_dia,
                                   v.base_semanal_dias)      AS dias_trabajo_semana5,
...
def main() -> int:
    ...
    con.create_function("dias_en_semana_de_cinco", 3,
                        dias_en_semana_de_cinco)
