...
# --- 7 · el CSV convertido y la metrica no pueden divergir -----------------
# EL DEFECTO QUE ESTA PRUEBA FIJA, y lo encontro una evaluacion externa del
# paquete YA PUBLICADO. La conversion a semana comun estaba escrita dos veces:
# en Python dentro de la metrica y en SQL dentro del exportador. El SQL cableaba
# base cinco para `habil` y base seis para `werktage`, cuando las dos etiquetas
# describen lo mismo y las dos deben leer la base de la norma.
#
#     Colombia   15 habiles sobre 6   ->  informe 12,5   CSV publicado 15,0
#     Tailandia   6 habiles sobre 6   ->  informe  5,0   CSV publicado  6,0
#
# Alemania convertia —24 Werktage sobre 6 dan 20— y Colombia no, siendo el mismo
# caso. Dos salidas nuestras decian cosas distintas del mismo pais.
#
# LA PRUEBA NO COMPARA EL CSV CONTRA LA METRICA, y esa distincion importa: no
# tienen por que coincidir. El CSV convierte la CIFRA DEL TEXTO LEGAL; la metrica
# resuelve la titularidad del trabajador de referencia, que en las unidades con
# escala de antiguedad es otra cosa —Dominicana da 11,7 aqui y 12,8 alla, y las
# dos son correctas—. Lo que se comprueba es que cada fila del CSV sea
# exactamente lo que la unica funcion de conversion devuelve para SUS PROPIOS
# campos. Comparar contra la metrica habria hecho fallar a Dominicana por una
# diferencia legitima.
import csv as _csv2

# LA ARITMETICA SE ESCRIBE AQUI, no se importa. Mi primera version de esta prueba
# comparaba el CSV contra `conversion.dias_en_semana_de_cinco` — la funcion que
# PRODUCE el CSV. Reintroduje el defecto original a mano para comprobarla y la
# prueba siguio pasando: al cambiar la funcion se movieron los dos lados a la vez.
#
# **Una prueba que llama al codigo que prueba no prueba nada.** Es la misma
# leccion que ya me mordio hoy leyendo mal la salida de otra compuerta: no basta
# con mirar, hay que comprobar que lo que mira, mira lo que crees.
#
# Asi que la regla va escrita en la prueba, en aritmetica desnuda y derivada del
# significado: un dia contado por la norma vale una fraccion de su semana legal,
# y en una semana comun de cinco eso son `dias * 5 / base`. Si alguien cambia la
# funcion, esto discrepa.
def _esperado(dias, tipo, base):
    d = float(dias)
    b = float(base) if base else 5.0
    return round({"calendario": d * 5.0 / 7.0,
                  "semanas": d * 5.0,
                  "habil": d * 5.0 / b,
                  "werktage": d * 5.0 / b}[tipo], 1)

_bases = {}
for f in _csv2.DictReader((EXPORT / "vacaciones.csv").open(encoding="utf-8")):
    _bases[(f["iso3"], f["dias_texto_legal"])] = f["base_semanal_dias"] or None

malas = []
for r in _csv2.DictReader((EXPORT / "vacaciones_convertido.csv").open(encoding="utf-8")):
    base = _bases.get((r["iso3"], r["dias_texto_legal"]))
    esp = _esperado(r["dias_texto_legal"], r["tipo_de_dia"], base)
    if abs(float(r["dias_trabajo_semana5"]) - esp) > 0.05:
        malas.append("%s: el CSV dice %s y la regla da %.1f (%s sobre base %s)"
                     % (r["iso3"], r["dias_trabajo_semana5"], esp,
                        r["tipo_de_dia"], base or "sin declarar"))
ok(not malas, "el CSV convertido cumple la regla dias*5/base, escrita aparte",
   "; ".join(malas[:5]))

# Y LOS CUATRO CASOS QUE NOS MORDIERON, por su valor. Valen menos que la regla
# —fijan casos y no la clase— y se conservan porque son numeros que ya salieron
# publicados mal.
_FIJOS = {"COL": 12.5, "THA": 5.0, "DOM": 11.7, "DEU": 20.0, "PER": 21.4}
_pub = {r["iso3"]: float(r["dias_trabajo_semana5"])
        for r in _csv2.DictReader(
            (EXPORT / "vacaciones_convertido.csv").open(encoding="utf-8"))}
_mal = ["%s: %s en vez de %s" % (i, _pub.get(i), v)
        for i, v in sorted(_FIJOS.items()) if abs(_pub.get(i, -1) - v) > 0.05]
ok(not _mal, "los cuatro casos que se publicaron mal dan su valor correcto",
   "; ".join(_mal))
